...
bool MemoryManager::copyFrom(const void *src, uint32_t dest, uint32_t len) {
  for (uint32_t i = 0; i < len; ++i) {
    ...
    this->setByte(dest + i, (( uint8_t* )src)[i], -1);
  }
  return true;
}

bool MemoryManager::setByte(uint32_t addr, uint8_t val, uint64_t pc, uint32_t* cycles) {
...
uint8_t MemoryManager::getByte(uint32_t addr, uint64_t pc, uint32_t* cycles) {
    if (!this->isAddrExist(addr)) {
...
bool MemoryManager::setShort(uint32_t addr, uint16_t val, uint64_t pc, uint32_t* cycles) {
    if (!this->isAddrExist(addr)) {
...
bool MemoryManager::setInt(uint32_t addr, uint32_t val, uint64_t pc, uint32_t* cycles) {
    if (!this->isAddrExist(addr)) {
...
bool MemoryManager::setLong(uint32_t addr, uint64_t val, uint64_t pc, uint32_t* cycles) {
    if (!this->isAddrExist(addr)) {
